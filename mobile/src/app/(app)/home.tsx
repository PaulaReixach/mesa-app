...
function pickRecommendation(
  dashboardData: GroupDashboardData[],
): HomeRecommendation | null {
  const candidates = dashboardData.flatMap(({ group, restaurants }) => {
    return restaurants.map(restaurant => ({ group, restaurant }));
  });
...
export default function HomeScreen() {
  ...
  const loadHome = useCallback(async (refreshing = false): Promise<void> => {
    ...
    try {
      ...
      const memberMap = dashboardData.reduce<Record<string, GroupMember[]>>(
        (result, item) => {

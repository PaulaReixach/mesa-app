...
type GroupModalMode = 'GROUP' | 'RESTAURANT';
...
function QuickFilter({
  ...
  return (
    ...
    >
      <SymbolView name={icon} size={14} tintColor={tintColor} />
      <Text
...
export default function MapScreenUnified() {
  ...
  const [groupModalMode, setGroupModalMode] = useState<GroupModalMode>('GROUP');
  ...
  const aggregatedRestaurants = useMemo<MapRestaurantItem[]>(() => {
    ...
    return [...groupedRestaurants.values()].map(group => {
      const representative = group[0];
      const memberships: RestaurantMembership[] = group.map(item => ({
    ...
    return aggregatedRestaurants
      .filter(restaurant => {
        ...
        const searchable = normalizeSearch([
          ...
          restaurant.city,
          restaurant.country,
          ...restaurant.memberships.map(item => item.groupName),
        ].filter(Boolean).join(' '));
  ...
  async function toggleFavorite(restaurant: MapRestaurantItem): Promise<void> {
    ...
    try {
      ...
      await Promise.all(restaurant.memberships.map(membership => (
  ...
  const groupCaption = selectedRestaurant && selectedRestaurant.memberships.length > 1
    ...
    : 'Abrir grupo';
  const groupModalTitle = groupModalMode === 'RESTAURANT'
    ? 'Elige un grupo'
    : 'Grupos del restaurante';
  const groupModalSubtitle = groupModalMode === 'RESTAURANT'

...
type RestaurantGroupMembership = Pick<
  MapRestaurant,
...
const USER_REGION_DELTA = 0.035;
const RESTAURANT_REGION_DELTA = 0.022;
...
export default function MapScreen() {
  ...
  const groupCountByRestaurantId = useMemo(() => {
    const groupIdsByRestaurant = new Map<string, Set<string>>();

    restaurants.forEach(restaurant => {
      const groupIds = groupIdsByRestaurant.get(restaurant.restaurantId)
        ?? new Set<string>();
      groupIds.add(restaurant.groupId);
      groupIdsByRestaurant.set(restaurant.restaurantId, groupIds);
    });

    return new Map(
      [...groupIdsByRestaurant.entries()].map(([restaurantId, groupIds]) => [
        restaurantId,
        groupIds.size,
      ]),
    );
  ...
  const selectedGroupMemberships = useMemo<RestaurantGroupMembership[]>(() => {
    ...
    return restaurants
      .filter(restaurant => restaurant.restaurantId === selectedRestaurant.restaurantId)
      .filter(restaurant => {
        if (seenGroupIds.has(restaurant.groupId)) {
          return false;
        }

        seenGroupIds.add(restaurant.groupId);
        return true;
      })
      .map(restaurant => ({
  ...
  useEffect(() => {
    if (
      !isMapReady
      || isLoading
      || hasPositionedInitialCameraRef.current
    ) {
      return;
    }
  ...
  function handleGroupsPress(): void {
    if (selectedGroupMemberships.length === 1) {
      openGroup(selectedGroupMemberships[0].groupId);
      return;
    }
  ...
  function focusRestaurant(restaurant: MapRestaurantItem): void {
    ...
    mapRef.current?.animateToRegion(
      {
        latitude: restaurant.latitude,
        latitudeDelta: RESTAURANT_REGION_DELTA,
        longitude: restaurant.longitude,
        longitudeDelta: RESTAURANT_REGION_DELTA,
      },
      350,
    );
  }
  ...
  async function toggleFavorite(restaurant: MapRestaurantItem): Promise<void> {
    ...
    const previousStatus = previousStatusByRestaurantRef.current[
  ...
  const selectedGroupLabel = selectedGroupMemberships.length > 1
    ? `Guardado en ${selectedGroupMemberships.length} grupos`
    : selectedGroupMemberships[0]?.groupName ?? 'Ver grupo';
  const selectedGroupCaption = selectedGroupMemberships.length > 1
          ...
          <View {...panResponder.panHandlers} style={styles.dragZone}>
            ...
            {selectedRestaurant ? (
              <View style={styles.selectedCard}>
                ...
                {selectedGroupMemberships.length > 0 ? (
                  <Pressable
                    accessibilityRole="button"

...
function GroupFilterRow({
  ...
  const meta = group
    ? [privacy, group.city, `${restaurantCount} ${restaurantCount === 1 ? 'restaurante' : 'restaurantes'}`]
        .filter(Boolean)
        .join(' · ')
    ...
    >
      <View
        style={[
          ...
          selected ? groupFilterStyles.groupIconSelected : null,
        ]}
      >
        <SymbolView
...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    hasPositionedInitialCameraRef.current = true;
  }, [fitRestaurants, isLoading, isMapReady, userLocation, visibleRestaurants]);

  const filterSignature = `${selectedGroupId ?? 'ALL'}:${statusFilter}:${favoritesOnly}:${normalizeSearch(searchQuery)}`;
  ...
  useEffect(() => {
    ...
    previousFilterSignatureRef.current = filterSignature;
    const timeoutId = setTimeout(() => fitRestaurants(visibleRestaurants), 120);

    return () => clearTimeout(timeoutId);
  }, [filterSignature, fitRestaurants, isLoading, isMapReady, visibleRestaurants]);
  ...
  const allActive = statusFilter === 'ALL'

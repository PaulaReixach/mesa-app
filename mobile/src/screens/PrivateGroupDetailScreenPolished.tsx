...
function isStatusFilter(value: RestaurantFilter): boolean {
...
export default function PrivateGroupDetailScreenPolished() {
      ...
      >
        ...
        {!loading && error ? (
          <View style={styles.standaloneError}>
            <MaterialIcons color={colors.primary} name="wifi-off" size={28} />
            <Text style={styles.errorTitle}>No hemos podido cargar el grupo</Text>
          ...
          <>
            ...
            <View style={styles.sheet}>
              <View style={styles.summary}>
                <GroupHeading
                  city={group.city}
                  privacyLabel={group.privacy === 'PRIVATE' ? 'Grupo privado' : 'Grupo público'}
                  title={group.name}
                />

                <View style={styles.statsRow}>
                  <GroupStat kind="restaurants" label="restaurantes" value={restaurants.length} />
                  <GroupStat kind="members" label="miembros" value={members.length} />
                  <GroupStat kind={thirdStat.kind} label={thirdStat.label} value={thirdStat.value} />
                </View>
              ...
              <View style={styles.tabContent}>
                {activeTab === 'restaurants' ? (
                  <>
                    <View style={styles.restaurantTools}>
                      <View style={styles.searchBar}>
                        ...
                        {restaurantSearchQuery ? (
                          ...
                          >
                            <MaterialIcons color={colors.muted} name="cancel" size={18} />
                          </Pressable>
                        ) : null}
                      </View>
                      ...
                      >
                        {restaurantFilters.map(filter => {
                          ...
                          const count = filter.value === 'FAVORITES' ? favoriteCount : null;
                    ...
                    {isOwner && group.privacy === 'PUBLIC' && pendingProposalCount > 0 ? (
                      <GroupInfoBanner
                        actionLabel="Revisar"
                        icon={{ ios: 'tray.full.fill', android: 'inbox', web: 'inbox' }}
                        onPress={openProposals}
                        subtitle="Restaurantes propuestos por colaboradores."
                        title={`${pendingProposalCount} ${pendingProposalCount === 1 ? 'propuesta pendiente' : 'propuestas pendientes'}`}
                      />
                    ) : null}
                      ...
                      <View style={styles.restaurantList}>
                        {filteredRestaurants.map(item => (
                          <GroupRestaurantRowPolished
                            item={item}
                            key={item.id}
                            onFavoritePress={updatingFavoriteId === item.id ? undefined : () => void toggleFavorite(item)}
